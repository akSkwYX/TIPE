...
let correct_determiner_noun determiner noun =
  let checkings_fun token args =
    Checkings.check_determiner_noun (Node (token, [Leaf token])) args
  in
  let construct_tree_fun tags token args =
    Node (TEMP (Syntax_tree.st_get_word args ^ " " ^ Token.get_word token, tags),
          [args; Node (token, [Leaf token])])
  in
  correction checkings_fun construct_tree_fun noun determiner
