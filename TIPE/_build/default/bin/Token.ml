		...
		| word :: t -> 
			...
			if is_word then
				let rec match_information i possibility =
					match i with
					| [] -> possibility
					| ("D"::complementary_information) :: tl -> match_information tl (Token ( Word_classe.Determinant, (word, complementary_information) ) :: possibility)
					| ("N"::complementary_information) :: tl -> match_information tl (Token ( Word_classe.Nom, (word, complementary_information) ) :: possibility)
					| ("A"::complementary_information) :: tl -> match_information tl (Token ( Word_classe.Adjectif, (word, complementary_information) ) :: possibility)
					| ("V"::complementary_information) :: tl -> match_information tl (Token ( Word_classe.Verbe, (word, complementary_information) ) :: possibility)
					| ("Os"::complementary_information) :: tl -> match_information tl (Token ( Word_classe.Pronom_sujet, (word, complementary_information) ) :: possibility)
