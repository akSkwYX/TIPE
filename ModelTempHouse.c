...
void nextStep(cell **map, list** lists)
{
	updateWalls(map, lists[0]);
	updateStructures(map, lists[3]);
	updateIntAirs(map, lists[1]);
	updateExtAirs(map, lists[2]);
}


/* Initialise la carte des cellules avec leur différents paramètres */

cell** initialize(int height, int width, int* house, structure* structures, int nbrStructure, float* temps, float* CTherVol, float* lambdas, float* surfaces, float* epaisseurs, list**	lists)
{
...
int main()
{
	...
	structure init_structure[3] = {window1, window2, door};
	float temps[4] = {/* wall */ 25.0, /* AirInt */ 16.0, /* AirExt */ 25.0, /* window */ 25.0};
	float CTherVol[4] = {/* wall */ 2400000, /* AirInt */ 1256, /* AirExt */ 1256, /* window */ 17000};
	float lambdas[4] = {/* wall */ 0.061 , /* AirInt */ 0.025 , /* AirExt */ 0.025, /* window */ 1.0};
	float surfaces[4] = {/* wall */ 1 , /* AirInt */ 1 , /* AirExt */ 1, /* window */ 1};
	float epaisseurs[4] = {/* wall */ 0.394 , /* AirInt */ 1 , /* AirExt */ 1, /* window */ 0.1};
    int lapsTime = 86400;
    ...
    cell** map = initialize(height, width, house, init_structure, nbrStructure, temps, CTherVol, lambdas, surfaces, epaisseurs, lists);
